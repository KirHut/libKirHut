...
struct KH_EXPORT Input
{
     ...
     *
     * By default, this returns ture if bytesReady() returns a value greater than zero. Generally speaking, this is a
     ...
     * read from or the end of file/data has been reached. By default, goodIn() returns !failed() && moreContent(), and
     * any subclass implementation should return an equivalent functionality (as in, if goodIn() returns false, then
     * either failed() returns true or moreContent() returns false).
     *
    ...
    /*!
     * Take in the next "token" in the input stream, which works identically to std::istream >> std::string.
     *
 ...
 *
 * Only the printV(string_view) and failedOut() methods need to be implemented, but it is highly encouraged that you
...
class KH_EXPORT SeekableInput : public Input
{
    using enum Relation;

    virtual bool isFloating() const noexcept;
    virtual bool setFloating(bool shouldFloat = true);

    virtual SeekPos tellIn() const noexcept               = 0;
    virtual void seekIn(i64 off, Relation from = CURRENT) = 0;
};

class KH_EXPORT SeekableOutput : public Output
{
    using enum Relation;
...
template <typename... Args>
inline void print(std::format_string<Args...> str, Args &&...args)
{
    out().print(str, forward<Args>(args)...);
}

template <typename... Args>
inline void println(std::format_string<Args...> str, Args &&...args)
{
    out().println(str, forward<Args>(args)...);
}

/*!
 * \brief The BufferedOutput class
 */
class KH_EXPORT BufferedOutput : public SeekableOutput
{
};

class KH_EXPORT StringOutput : public SeekableOutput
{
public:
};

class KH_EXPORT FileOutput : public SeekableOutput
{
public:
};

class KH_EXPORT FileReadStream : public SeekableInput
{
public:
};

class KH_EXPORT FileStream : public SeekableInput, public SeekableOutput
{
};
